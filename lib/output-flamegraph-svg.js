...
FlameGraphEmitter.prototype.draw = function (callback)
{
	...
	xml.emitCData(
	    ...
	    '   console.log(arguments); $(self).popover("show");\n' +
	    ' }\n' +
	    ' function c(self) {\n' +
	    '   console.log(arguments); $(self).popover("hide");' +
	    ' }'
	);
	xml.emitEnd('script');
	...
	for (var ident in this.fge_boxes) {
		...
		if (func.length === 0 && depth === 0) {
			...
			pct = ((100 * nsamples) / this.fge_time).toFixed(2);
		}
		info = '<div class="flmg-info">' + nsamples + ' samples (' + pct + '%)</div>';

		var color = this.color(depth, nsamples);

		var onmouseover = 's(' + info + ', this)';
		var onmouseout  = 'c(this)';
	...
	xml.emitStart('script', { 'type': 'text/ecmascript' });
	xml.emitCData(
	    '	$(".flmg-elem").popover({container: "body", html: true});'
	);
	xml.emitEnd('script');
